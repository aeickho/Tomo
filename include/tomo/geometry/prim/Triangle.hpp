    ...
    namespace prim
    {
      ...
      struct TriangleConcept : public SlicableObject
      {
        ...
        void slice(Slices& _slices) const
        {
          ...
          if (_sliceA->posZ_ > _sliceB->posZ_)
          {
            std::swap(_sliceA,_sliceB);
            std::swap(_Ait,_Bit);
            std::swap(A,B);
          }
          if (_sliceB->posZ_ > _sliceC->posZ_)
          {
            std::swap(_sliceB,_sliceC);
            std::swap(_Bit,_Cit);
            std::swap(B,C);
          }
          if (_sliceA->posZ_ > _sliceB->posZ_)
      ...
      struct Triangle : public TriangleConcept<Point3f>
      {
        ...
        const Point3f& v1() const
        {
          return v_[1];
        }
        const Point3f& v2() const
        {
          return v_[2];
        }
