...
namespace tomo
{
  namespace geometry
  {
    namespace prim
    {
      bool Plane::intersect(ray_type& _ray, scalar_type& _tNear, scalar_type&_tFar, vec_type* _normal) const
      {
        scalar_type dn = dot(_ray.dir(),normal_);
        if (dn == 0.0f) return false;

        vec_type org = _ray.org() - center_;

        scalar_type d = -dot(normal_,org) / dn;
        if (d < 0) return false;

        vec_type iPoint = _ray.org() +_ray.dir() *d - center_;
